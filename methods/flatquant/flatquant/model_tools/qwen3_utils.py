...
from transformers.models.qwen3.modeling_qwen3 import Qwen3MLP, Qwen3Attention, logger, \
                                                     apply_rotary_pos_emb, eager_attention_forward


class FlatQuantQwen3MLP(torch.nn.Module):
    def __init__(self, args, module: Qwen3MLP):
        super().__init__()
        self.args = args
...
class FlatQuantQwen3Attention(Qwen3Attention):
    ...
    def forward(self, hidden_states, position_embeddings, attention_mask, past_key_value=None, 
...
def apply_flatquant_to_qwen3(args, model):
    ...
    for layer in range(model.config.num_hidden_layers):
        # attn
        model.model.layers[layer].self_attn = FlatQuantQwen3Attention(args, model.model.layers[layer].self_attn)
        # mlp
        model.model.layers[layer].mlp = FlatQuantQwen3MLP(args, model.model.layers[layer].mlp)
    return model
