...
def cali_flat_quant(args, model, dataloader, dev, logger, start_layer_idx=0):
    ...
    for i in range(num_train_layer):
        ...
        if args.cali_trans:
            trans_linear_names = [f"trans_list.{tp}.linear" for tp in range(args.tp)] + ["trans.linear"]
            trained_params.append({"params": get_n_set_parameters_byname(layer, trans_linear_names), "lr": args.flat_lr})
            paras_name.extend(trans_linear_names)
        if args.add_diag:
            trained_params.append({"params": get_n_set_parameters_byname(layer, ["diag_scale", ]), "lr": args.flat_lr})
            paras_name.append("diag_scale")
        if args.lwc:
        ...
        if args.epochs > 0 and i >= start_layer_idx:
