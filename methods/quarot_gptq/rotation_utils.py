...
def fuse_layer_norms(model):
    ...
    model_utils.get_pre_head_layernorm(**kwargs).weight.data.fill_(1.0)
...
def rotate_mlp_output(layer, Q, model_type, tp):
    ...
    W_ = W.weight.data.to(device=utils.DEV, dtype=torch.float64)
    W.weight.data = torch.matmul(Q.T, W_).to(device="cpu", dtype=dtype)
    apply_exact_had_to_linear(W, had_dim=-1 if tp == 1 else in_dim // tp, output=False) #apply exact (inverse) hadamard on the weights of mlp output
    if W.bias is not None:
        b = W.bias.data.to(device=utils.DEV, dtype=torch.float64)
        W.bias.data = torch.matmul(Q.T, b).to(device="cpu", dtype=dtype)
...
def matmul_hadU_cuda_had(X, hadK, transpose=False):
    ...
    '''
    from fast_hadamard_transform import hadamard_transform
    from ..utils.hadamard_utils import get_had172
    n = X.shape[-1]
    K = hadK.shape[-1]
...
def rotate_ov_proj(layer, model_type, head_num, head_dim, tp):
    ...
    apply_exact_had_to_linear(o_proj, had_dim=-1 if tp == 1 else head_dim, output=False)
...
def rotate_model(model, args):
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        ...
        rotate_mlp_output(layers[idx], Q, model_type, args.tp)
        rotate_ov_proj(layers[idx], model_type, num_heads, head_dim, args.tp)
